<template>
  <div class="common-style-table">
    ...
    >
      <template slot="sex" slot-scope="{ index, record }">
        <span>{{ record.sex===0?'男':'女' }}</span>
      </template>
       <template slot="gradleText" slot-scope="{ index, record }">
        <span>{{ initGradeText(record.gradle) }}</span>
      </template>
